...
describe "Migration.descendants" do
  before do
    Migrator::Migration.descendants.clear
  end

  it "should include Migration subclasses" do
    @class = Class.new(Migrator::Migration)

    Migrator::Migration.descendants.must_equal [@class]
  end

  it "should include Migration subclasses in order of creation" do
    @c1 = Class.new(Migrator::Migration)
    @c2 = Class.new(Migrator::Migration)
    @c3 = Class.new(Migrator::Migration)

    Migrator::Migration.descendants.must_equal [@c1, @c2, @c3]
  end
  ...
  it "should include SimpleMigration instances created by migration DSL" do
    ...
    Migrator::Migration.descendants.must_equal [i1, i2, i3]
  end
end
...
describe "Migration.apply" do
  ...
  it "should raise for an invalid direction" do
    proc {Migrator::Migration.apply(@db, :hahaha)}.must_raise(ArgumentError)
  end

  it "should apply the up and down directions correctly" do
    m = Class.new(Migrator::Migration) do
...
describe "Reversible Migrations with Sequel.migration{change{}}" do
  ...
  it "should raise in the down direction if migration uses unsupported method" do
    ...
    m.apply(@db, :up)
    proc{m.apply(@db, :down)}.must_raise(Migrator::Error)
  end
...
describe "Migrator::Migrator.migrator_class" do
  it "should return IntegerMigrator if not using timestamp migrations" do
    Migrator::Migrator.migrator_class("spec/files/integer_migrations").must_equal Migrator::IntegerMigrator
  end

  it "should return TimestampMigrator if using timestamp migrations" do
    Migrator::Migrator.migrator_class('spec/files/timestamped_migrations').must_equal Migrator::TimestampMigrator
  end

  it "should return self if run on a subclass" do
    Migrator::IntegerMigrator.migrator_class("spec/files/timestamped_migrations").must_equal Migrator::IntegerMigrator
    Migrator::TimestampMigrator.migrator_class("spec/files/integer_migrations").must_equal Migrator::TimestampMigrator
  end
end

describe "Migrator::IntegerMigrator" do
  before do
    dbc = Class.new(Migrator::Mock::Database) do
  ...
  it "should raise and error if there is a missing integer migration version" do
    proc{Migrator::Migrator.apply(@db, "spec/files/missing_integer_migrations")}.must_raise(Migrator::Migrator::Error)
  end
  ...
  it "should not raise and error if there is a missing integer migration version and allow_missing_migration_files is true" do
    Migrator::Migrator.run(@db, "spec/files/missing_integer_migrations", :allow_missing_migration_files => true)
    @db.sqls.last.must_equal "UPDATE schema_info SET version = 3"
    Migrator::Migrator.run(@db, "spec/files/missing_integer_migrations", :allow_missing_migration_files => true, :target=>0)
  ...
  it "should raise and error if there is a duplicate integer migration version" do
    proc{Migrator::Migrator.apply(@db, "spec/files/duplicate_integer_migrations")}.must_raise(Migrator::Migrator::Error)
  end

  it "should raise an error if there is an empty migration file" do
    proc{Migrator::Migrator.apply(@db, "spec/files/empty_migration")}.must_raise(Migrator::Migrator::Error)
  end

  it "should raise an error if there is a migration file with multiple migrations" do
    proc{Migrator::Migrator.apply(@db, "spec/files/double_migration")}.must_raise(Migrator::Migrator::Error)
  end
  ...
  it "should add a column name if it doesn't already exist in the schema_info table" do
    ...
    Migrator::Migrator.apply(@db, @dirname)
  end
  ...
  it "should automatically create the schema_info table with the version column" do
    ...
    Migrator::Migrator.run(@db, @dirname, :target=>0)
  ...
  it "should allow specifying the table and columns" do
    ...
    Migrator::Migrator.run(@db, @dirname, :target=>0, :table=>:si, :column=>:sic)
  ...
  it "should be able to tell whether there are outstanding migrations" do
    Migrator::Migrator.is_current?(@db, @dirname).must_equal false
    Migrator::Migrator.apply(@db, @dirname)
    Migrator::Migrator.is_current?(@db, @dirname).must_equal true
  end

  it "should have #check_current raise an exception if the migrator is not current" do
    proc{Migrator::Migrator.check_current(@db, @dirname)}.must_raise(Migrator::Migrator::NotCurrentError)
    Migrator::Migrator.apply(@db, @dirname)
    Migrator::Migrator.check_current(@db, @dirname)
  end

  it "should apply migrations correctly in the up direction with target" do
    Migrator::Migrator.apply(@db, @dirname, 2)
  ...
  it "should apply migrations correctly in the up direction with target and existing" do
    Migrator::Migrator.apply(@db, @dirname, 2, 1)
  ...
  it "should apply migrations correctly in the down direction with target" do
    ...
    @db.version.must_equal 3
    Migrator::Migrator.apply(@db, @dirname, 0)
  ...
  it "should apply migrations correctly in the down direction with target and existing" do
    Migrator::Migrator.apply(@db, @dirname, 1, 2)
  ...
  it "should return the target version" do
    Migrator::Migrator.apply(@db, @dirname, 3, 2).must_equal 3
    Migrator::Migrator.apply(@db, @dirname, 0).must_equal 0
    Migrator::Migrator.apply(@db, @dirname).must_equal 3
  end

  it "should use IntegerMigrator if IntegerMigrator.apply called, even for timestamped migration directory" do
    proc{Migrator::IntegerMigrator.apply(@db, "spec/files/timestamped_migrations")}.must_raise(Migrator::Migrator::Error)
  end
end

describe "Migrator::TimestampMigrator" do
  before do
    sequel_migration_version = 0
    @dsc = dsc = Class.new(Migrator::Mock::Dataset) do
      self::FILES =[]
  ...
  it "should raise an error if there is an empty migration file" do
    proc{Migrator::TimestampMigrator.apply(@db, "spec/files/empty_migration")}.must_raise(Migrator::Migrator::Error)
  end

  it "should raise an error if there is a migration file with multiple migrations" do
    proc{Migrator::TimestampMigrator.apply(@db, "spec/files/double_migration")}.must_raise(Migrator::Migrator::Error)
  end
  ...
  it "should raise an exception if the migrator is not current" do
    ...
    @dir = 'spec/files/interleaved_timestamped_migrations'
    proc{@m.check_current(@db, @dir)}.must_raise(Migrator::Migrator::NotCurrentError)
  end
  ...
  it "should raise error for applied migrations not in file system" do
    ...
    proc{@m.apply(@db, @dir, 0)}.must_raise(Migrator::Migrator::Error)
    [:schema_migrations, :sm1111, :sm2222, :sm3333].each{|n| @db.table_exists?(n).must_equal true}
    @db[:schema_migrations].select_order_map(:filename).must_equal %w'1273253849_create_sessions.rb 1273253851_create_nodes.rb 1273253853_3_create_users.rb'
  end
  ...
  it "should raise error missing column name in existing schema_migrations table" do
    @dir = 'spec/files/timestamped_migrations'
    @m.apply(@db, @dir)
    proc{@m.run(@db, @dir, :column=>:fn)}.must_raise(Migrator::Migrator::Error)
  end
